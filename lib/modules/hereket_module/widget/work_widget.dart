...
class _WorkWidgetState extends State<WorkWidget> {
  final hereketWorkCtrl = Get.find<HereketWorkController>();
  final ApplyController applyCtrl = Get.find<ApplyController>();
  // final HereketPlaniController hereketPlaniCtrl =
  ...
  final rQiymetCtrl = TextEditingController();
  final searchItemCtrl = TextEditingController();
  final noteController = TextEditingController();

  bool get isBarcode =>
      hereketWorkCtrl.selectedWorkItem?.name.toLowerCase() == 'barkod';
  bool get onlyNumbers => hereketWorkCtrl.selectedWorkItem?.name.toLowerCase() == 'no';
  ...
  void _onAmountFocus() {
    if (amountFocus.hasFocus) {
      // hereketPlaniCtrl.setActiveController(amountController);
      hereketWorkCtrl.setActiveController(amountController);
      _scrollToEnd();
    }
  }
  ...
  void _onRQiymetFocus() {
    if (rQiymetFocus.hasFocus) {
      // hereketPlaniCtrl.setActiveController(rQiymetCtrl);
      hereketWorkCtrl.setActiveController(rQiymetCtrl);
    }
  }

  void _onSearchFocus() {
    if (searchFocusNode.hasFocus) {
      hereketWorkCtrl.setActiveController(searchItemCtrl);
      // hereketWorkCtrl.setActiveController(rQiymetCtrl);
    }
  }
  ...
  void _init() async {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // _initFromInitial();
      noteController.text = 'Multi';
      // hereketPlaniCtrl.activeController = searchItemCtrl;
      hereketWorkCtrl.activeController = searchItemCtrl;
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                  ...
                  builder: (HereketWorkController ctrl) {
                    ...
                    return Expanded(
                      child: TextField(
                        ...
                        readOnly: (isBarcode && !allowKeyboard)  || onlyNumbers, // barkod seçilib, allowKeyboard false → klaviatura gizli
                        showCursor: true,
                        autofocus: true,
                        ...
                        keyboardType: onlyNumbers
                            ? TextInputType.none
                            : (isBarcode && !allowKeyboard ? TextInputType.none : TextInputType.text),
                              ...
                              SizedBox(
                                ...
                                child: IconButton(
                                  ...
                                  onPressed: () {
                                    ...
                                    performSearch(searchItemCtrl.text);
                                  },
                                ),
                              ),
                              if (showClear)
            ...
            SizedBox(
              ...
              child: GetBuilder<HereketWorkController>(
                builder: (HereketWorkController ctrl) {
                  ...
                  // if (ctrl.qaimeler.length == 1 && searchFocusNode.hasFocus) {
                  //   Future.delayed(const Duration(milliseconds: 100), () {
                  //     amountFocus.requestFocus();
                  //   });
                  // }
            ...
            // SizedBox(
            //   height: MediaQuery.of(context).size.height * 0.3, // ekranın 30%-i
            //   child: Row(
            //     children: [
            //       Expanded(
            //         flex: 3,
  ...
  void performSearch(String query) {
    ...
    Future.delayed(const Duration(milliseconds: 100), () {
      if (hereketWorkCtrl.qaimeler.length == 1) {
        amountFocus.requestFocus();
      } else {
        searchFocusNode.requestFocus(); // əks halda search-a qayıt
      }
    });
  }
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                //eger add secilibse
                debugPrint('qaime:${ hereketWorkCtrl.selectedQaimeDto}');

                if (!Validators.show(
                  hereketWorkCtrl.selectedQaimeDto != null,
                  "Əməliyyat üçün mal seçilməyib!",
                ))
                  return;
                // final int? amount = int.tryParse(amountController.text);
                if (!Validators.number(
                  amountController,
                  "Miqdar düzgün daxil edilməyib!",
                ))
                  return;
                // ❗ 2. PRICE yoxla (rQiymetCtrl)
                if (widget.hereketId != 5) {
                  if (!Validators.number(
                    rQiymetCtrl,
                    "Qiymət düzgün daxil edilməyib!",
                  ))
                    return;
                }
                ...
                if (hereketWorkCtrl.getSelectedValue == '2') {
                  ...
                  final dto = HereketDPlan(
                    ...
                    hereketId: hereketWorkCtrl.selectedQaimeDto?.id,
                    amount: int.tryParse(amountController.text),
                    price: double.tryParse(rQiymetCtrl.text),
                    sellPrice: ctrl.selectedQaimeDto?.satis,
                  ...
                  int? dplanId = widget.dPlanId;
                  int? nov = hereketWorkCtrl.selectedQaimeDto?.id!;
                  double? price = widget.hereketId == 5
                      ? null
                      : double.tryParse(rQiymetCtrl.text);
                  // int? amount = int.tryParse(amountController.text);
                  String? note = hereketWorkCtrl.noteController.text;
                  ...
                  debugPrint('dto111->$dto');
                  // applyCtrl.fetchApplyDto(dplan, nov, price, amount, note);
                  // Get.back();
                  Get.bottomSheet(
                    ApplyWidget(
                      applyRequestDto: dto,
                      onNullSave: () {
                        ...
                        final dtoSave = HereketDPlan(
                          dPlanId: ctrl.selectedQaimeDto?.id,
                          hereketId: ctrl.selectedId,
                          amount: int.tryParse(amountController.text),
                          price: double.tryParse(rQiymetCtrl.text),
                          sellPrice: ctrl.selectedQaimeDto?.satis,
                          note: noteController.text,
                        ...
                        // hereketWorkCtrl.selectedValue = '2';
                        debugPrint('dtoSave...${dtoSave}');
                        ctrl.ok(dtoSave);
                      },
                    ),
                  ...
                  Get.bottomSheet<HereketDPlan>(
                    ...
                    if (dtoSave != null) {
                      ...
                      debugPrint('Updated DTO: $updatedDto');
